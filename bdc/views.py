...
def bdc_details(request, pk=None):
    ...
    show_export = "yes" if csv_file_path.exists() else "no"
...
def bdc_items_pdf(request, pk=None, fn=None):
    ...
    if os.path.exists(pdf_file_path):
        ...
        response['X-Accel-Redirect'] = f'/items/pdf/{ bdc.id }/{ pdf_file_name }'
...
def bdc_items_csv(request, pk=None, fn=None):
    ...
    if os.path.exists(csv_file_path):
        ...
        response = HttpResponse()
        response['Content-Type'] = 'application/octet-stream'
        response['X-Accel-Redirect'] = f'/items/csv/{ bdc.id }/{ csv_file_name }'
...
def create_items_pdf(art_id):
    # html_path = f"/tmp/report_{art_id}.html"

    # with open(html_path, "w") as f:
    #     f.write(render_html_for_art(art_id))

    # HTML(f"file://{html_path}").write_pdf(output_path)
    ...
    # Return as download
    pdf_file_name = f'eMarches.com-{ bdc.chrono }.pdf'
    pdf_file_path = f"{settings.MEDIA_ROOT}/pdf/article/{ pdf_file_name }"

    html_string = render_to_string("bdc/bdc-articles-pdf.html", context)
    # Convert to PDF
    HTML(html_string).write_pdf(pdf_file_path)
